...
<template>
  <header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
    <a href="/" class="d-flex align-items-center col-md-3 mb-2 mb-md-0 text-dark text-decoration-none">
      <p v-if="auth">
        Hi, {{message}}
        <span class="badge rounded-pill text-bg-info" v-if="role ==='ADMIN'">{{role}}</span>
        <span class="badge rounded-pill text-bg-warning" v-if="!(role ==='ADMIN')">{{role}}</span>
      </p>
    </a>
    ...
    <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
      ...
      <li v-if="auth && role ==='ADMIN'">
        <router-link to="/details" class="nav-link px-2 link-dark">Phiếu mượn trả</router-link>
      </li>
      <li v-if="auth && role ==='ADMIN'">
      ...
      <li v-if="auth">
        <router-link to="/profile" class="nav-link px-2 link-dark">Thông tin cá nhân</router-link>
      </li>
    </ul>
